...
function parse_contact_list(contactList) {
	...
	var sql = "INSERT OR REPLACE INTO Contact(id, fname, lname, email, homePhone, mobiphone, addr,is_cmp) VALUES (?,?,?,?,?,?,?, ?)";
	...
	db.transaction(function(tx) {
		for (var i = 0; i < length; i++) {
			var e = contactList[i];
			tx.executeSql(sql, [e.Id, e.FirstName, e.LastName, e.Email, e.Phone, e.Phone, e.Address, e.ContactType]);
		}
...
function parse_quotation_list(quotationList) {
	var sql = "INSERT OR REPLACE INTO Quotaion(id, Buyer,id_num,Age,Sex,Insure_Amount) VALUES (?,?,?,?,?,?)";
	var length = quotationList.length;
	alert("quotaion leng :" + length);
	for (var i = 0; i < length; i++) {
		...
		//Sync Contact
		db.transaction(function(tx) {
			tx.executeSql(sql, [e.Id, e.BuyerName, e.IdentityNumber, e.Age, sex, e.Amount]);
		}, function(err) {
